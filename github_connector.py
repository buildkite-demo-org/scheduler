...
# Constants
PRIVATE_KEY_PATH = Path("github_app_private_key.pem").resolve()  # Path to your GitHub App's private key
...
class GitHubConnector:
    ...
    def __init__(self):
        ...
        self.github = self.authenticate_as_github_app(GITHUB_APP_ID, self.installation.id, private_key)
        self.pr_id_to_check_run_id_dict: Dict[(int, str), int] = {}  # (pr_id, check_name) -> check_run_id
    ...
    def fetch_installation(cls) -> Installation:
        integration: GithubIntegration = GithubIntegration(GITHUB_APP_ID, cls.read_private_key(PRIVATE_KEY_PATH))
    ...
    @staticmethod
    def get_head_commit_sha_for_pull_request(repo: Repository, pull_request_id: int) -> str:
        pull_request: PullRequest = repo.get_pull(pull_request_id)
        return pull_request.head.sha

    # Function to update the check run on GitHub
    def update_checks_page(self, repo_name, check_name, pull_request_id, status, conclusion=None, output=None):
    ...
    @staticmethod
    def init_checks_page(repository: Repository, check_name: str, commit_sha: str, status: str,
                         output: Dict) -> int:
    ...
    @staticmethod
    def edit_checks_page(repository: Repository, check_run_id: int, status: str, conclusion: str, output: Dict):
...
def demo_github_connector():
    ...
    # Check
    output = {
        'title': 'Check is running',
        'summary': '...'
    }
    github.update_checks_page(repo_name=repo_name, check_name="check", pull_request_id=3, status="in_progress",
                              conclusion=None, output=output)
    ...
    output = {
        'title': 'Build Results',
        'summary': 'The build completed successfully.' if build_successful else 'The build failed.'
    }
    conclusion = "success" if build_successful else "failure"
    github.update_checks_page(repo_name=repo_name, check_name="check", pull_request_id=3, status="completed",
    ...
    # Gate
    output = {
        'title': 'Gate is running',
        'summary': '...'
    }
    github.update_checks_page(repo_name=repo_name, check_name="gate", pull_request_id=3, status="in_progress",
                              conclusion=None, output=output)
    ...
    conclusion = "success" if build_successful else "failure"
    github.update_checks_page(repo_name=repo_name, check_name="gate", pull_request_id=3, status="completed",
                              conclusion=conclusion, output=output)
